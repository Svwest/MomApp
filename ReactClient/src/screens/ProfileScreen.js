...
import { getUserByMail } from "../context/userContext"; // Подключите функцию getUserByMail из вашего контекста
...
export default ProfileScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        if (user) {
          const userData = await getUserByMail(user.email ,token);
  ...
  return (
    <View style={styles.container}>
      ...
      <Button  onPress={() => navigation.navigate("EditProfile", { loadedUser })} fontSize={80}>Edit Profile</Button>
